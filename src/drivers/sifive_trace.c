...
#define TRACE_REG8(offset)   (__METAL_ACCESS_ONCE((__metal_io_u8  *)TRACE_REG(offset)))
#define TRACE_REG16(offset)  (__METAL_ACCESS_ONCE((__metal_io_u16 *)TRACE_REG(offset)))
#define TRACE_REG32(offset)  (__METAL_ACCESS_ONCE((__metal_io_u32 *)TRACE_REG(offset)))
...
void __metal_driver_sifive_trace_init(struct metal_uart *trace, int baud_rate)
{
    ...
    // other mechanisms to make sure tracing is enabled. If we try to enable tracing here, it
    // will likely conflict with Freedom Studio, and they will just fight with each other.
